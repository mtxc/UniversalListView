...
public class MyAdapter extends UniversalAdapter<Data> {
	...
	public void updateItem(ViewHolder holder, Data t) {
		...
		((TextView)holder.getView(R.id.tv_praise)).setText("赞(" + t.getPraise() + ")");
		((TextView)holder.getView(R.id.tv_browse)).setText("浏览(" + t.getBrowse() + ")");
		((TextView)holder.getView(R.id.tv_comment)).setText("评论(" + t.getComment() + ")");
	}
